...
async function openFixed(){const m=$('fixedDetailModal');const id=m?.dataset?.productId||window.GYX_SELECTED_FIXED_PRODUCT?.id;let product=window.GYX_SELECTED_FIXED_PRODUCT?.id===id?window.GYX_SELECTED_FIXED_PRODUCT:null;if(!product)product=await loadProduct(id);if(!product){toast('当前方案暂时无法下单');return}openUnified(product,null)}
