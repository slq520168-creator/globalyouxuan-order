...
function ensureUser(){if(currentUser)return Promise.resolve(currentUser);if(!authPromise)authPromise=user().then(u=>{currentUser=u;return u}).finally(()=>{authPromise=null});return authPromise}
...
function openModalShell(product,match){currentProduct=product;currentMatch=match||null;setChineseLabels();$('orderName').value=currentUser?.user_metadata?.display_name||currentUser?.email?.split('@')[0]||'';$('orderEmail').value=currentUser?.email||'';$('orderPhone').value='';['orderName','orderEmail','orderPhone'].forEach(id=>{const e=$(id);if(e){e.readOnly=false;e.removeAttribute('aria-readonly');e.removeAttribute('tabindex')}});$('orderProductName').textContent=currentMatch?.answer?.title||currentProduct.product_name||'方案';$('orderProductDescription').textContent=currentMatch?.answer?.answer_summary||currentProduct.full_description||currentProduct.description||'';$('orderProductPrice').textContent=Number(currentProduct.product_price||0).toFixed(Number(currentProduct.product_price||0)%1?2:0);const cancel=$('cancelOrderButton');if(cancel)cancel.textContent='取消';msg('orderFormMessage','请确认订单信息。',true);step('details');const modal=$('orderModal');modal?.classList.add('show','open');modal?.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');document.body.style.overflow='hidden'}
async function hydrateProfile(){try{const u=await ensureUser();if(!u)return false;if(!$('orderEmail').value)$('orderEmail').value=u.email||'';if(!$('orderName').value)$('orderName').value=u.user_metadata?.display_name||u.email?.split('@')[0]||'';const pr=await db.from('profiles').select('display_name,phone').eq('user_id',u.id).maybeSingle();const p=pr.data||{};if(p.display_name&&!$('orderName').dataset.userEdited)$('orderName').value=p.display_name;if(p.phone&&!$('orderPhone').dataset.userEdited)$('orderPhone').value=p.phone;return true}catch{return false}}
function watchEdits(){['orderName','orderEmail','orderPhone'].forEach(id=>$(id)?.addEventListener('input',e=>{e.currentTarget.dataset.userEdited='1'},{once:true}))}
function openUnified(product,match=null){if(!product){toast('当前方案暂时无法下单');return}openModalShell(product,match);hydrateProfile();watchEdits()}
function openSearch(){const match=window.GYX_CURRENT_AI_MATCH;if(!match?.product||!match?.answer||!Array.isArray(match.selections)||match.selections.length<5){toast('请先完成五轮匹配');return}openUnified(match.product,match)}
async function openFixed(){const m=$('fixedDetailModal');const id=m?.dataset?.productId||window.GYX_SELECTED_FIXED_PRODUCT?.id;let product=window.GYX_SELECTED_FIXED_PRODUCT?.id===id?window.GYX_SELECTED_FIXED_PRODUCT:null;if(!product)product=await loadProduct(id);if(!product){toast('当前方案暂时无法下单');return}m?.classList.remove('show');document.body.classList.remove('fixed-detail-active');document.body.style.overflow='';openUnified(product,null)}
