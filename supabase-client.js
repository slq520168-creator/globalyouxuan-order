(() => {
  ...
  async function getVerifiedUser(force=false){
    ...
    verifiedPromise=(async()=>{try{const {data:sessionData}=await client.auth.getSession(),sessionUser=sessionData?.session?.user||null;if(!sessionUser){verifiedUser=null;verifiedAt=Date.now();return null}try{const {data,error}=await client.auth.getUser();if(!error&&data?.user){verifiedUser=data.user;verifiedAt=Date.now();return verifiedUser}}catch{}verifiedUser=sessionUser;verifiedAt=Date.now();return verifiedUser}catch{verifiedUser=null;verifiedAt=Date.now();return null}})();
