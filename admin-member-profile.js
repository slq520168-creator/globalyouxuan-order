(() => {
  ...
  async function edit(row) {
    ...
    const countryCodeInput = prompt(
      "国家/地区代码，例如 CN / KH / US / OTHER（可空）",
    ...
    if (
      phone_country_code &&
      phone_country_code !== "OTHER" &&
      !/^[A-Z]{2}$/.test(phone_country_code)
    ) {
      throw new Error("国家/地区代码格式不正确");
    }
    let phone_country_name = "";
    if (phone_country_code === "OTHER") {
      const countryNameInput = prompt(
        "国家/地区名称（可空）",
      ...
      try {
        ...
        phone_country_name = phone_country_code;
      }
    }
    const wechat = prompt("微信（可空）", row.wechat || "");
    if (wechat === null) return;
    const telegram = prompt("TG / Telegram（可空）", row.telegram || "");
    if (telegram === null) return;
    const whatsapp = prompt("WhatsApp（可空）", row.whatsapp || "");
    if (whatsapp === null) return;
    const locale = prompt("语言：zh-CN / en / km", row.locale || "zh-CN");
    ...
    alert("会员资料已更新");
    document.querySelector('[data-refresh="members"]')?.click();
  }
  document.addEventListener(
    "click",
    (e) => {
      const b = e.target.closest?.("[data-member-edit]");
      if (!b) return;
      const box = b.closest('[data-table="members"]');
      const row = box?._rows?.[Number(b.dataset.memberEdit)];
      if (!row) return;
      e.preventDefault();
