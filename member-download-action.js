(()=>{
  ...
  function render(){const list=document.getElementById('downloadList');if(!list)return;rows=sortNewestFirst(rows);list.replaceChildren();setCount(rows.filter(o=>Boolean(o.delivery_downloaded_at)).length);if(!rows.length){const e=document.createElement('div');e.className='empty-state';e.textContent=L('暂无可下载内容','No downloads available','មិនទាន់មានឯកសារទាញយក');list.append(e);return}rows.forEach(o=>list.append(card(o)));focusRequested()}
