(()=>{
  ...
  async function downloadAnswer(o,btn){
    ...
    try{
      ...
      toast(L('已下载，请到手机“文件/下载”中查看','Downloaded. Check Files/Downloads on your phone.','បានទាញយក។ សូមពិនិត្យ Files/Downloads នៅលើទូរស័ព្ទរបស់អ្នក'));
      btn.disabled=true;
  ...
  function card(o){const c=shell(o),actions=document.createElement('div');actions.className='order-actions';if(o.delivery_downloaded_at){const b=document.createElement('button');b.className='btn';b.type='button';b.disabled=true;b.textContent=L('下载已完成','Download complete','ការទាញយកបានបញ្ចប់');actions.append(b);c.append(actions);return c}if(String(o.status)==='delivered'&&(Number.isFinite(Number(o.answer_id))||o.source_module==='home_fixed'||String(o.product_id||'').startsWith('spare-time-'))){const b=document.createElement('button');b.className='btn';b.type='button';b.textContent=L('点击下载','Download','ចុចទាញយក');b.onclick=()=>downloadAnswer(o,b);actions.append(b)}else if(String(o.status)==='paid'){const p=document.createElement('p');p.className='muted';p.textContent=L('付款已确认，等待交付完成','Payment confirmed, awaiting delivery','បានបង់ប្រាក់ រង់ចាំការប្រគល់');c.append(p)}if(actions.childNodes.length)c.append(actions);return c}

  function downloadOpportunity(r,btn){const text=String(r.delivery_text_snapshot||''),bom=new Uint8Array([0xEF,0xBB,0xBF]),body=new TextEncoder().encode(text.replace(/\r?\n/g,'\r\n')),blob=new Blob([bom,body],{type:'text/plain;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=safeName(r.reward_title||L('接单机会','Opportunity','ឱកាសការងារ'))+'.txt';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);if(btn){btn.disabled=true;btn.textContent=L('下载已完成','Download complete','ការទាញយកបានបញ្ចប់')}toast(L('已下载，请到手机“文件/下载”中查看','Downloaded. Check Files/Downloads on your phone.','បានទាញយក។ សូមពិនិត្យ Files/Downloads នៅលើទូរស័ព្ទរបស់អ្នក'))}
