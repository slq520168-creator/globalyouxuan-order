(()=>{
  ...
  function downloadOpportunity(r){const text=String(r.delivery_text_snapshot||''),bom=new Uint8Array([0xEF,0xBB,0xBF]),body=new TextEncoder().encode(text.replace(/\r?\n/g,'\r\n')),blob=new Blob([bom,body],{type:'text/plain;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=safeName(r.reward_title||L('接单机会','Opportunity','ឱកាសការងារ'))+'.txt';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);toast(L('已下载接单内容','Opportunity downloaded','បានទាញយកមាតិកា'))}
  function opportunityCard(r){const c=document.createElement('article');c.className='order-card';c.dataset.gyxOpportunityRedemption=String(r.id||'');const top=document.createElement('div'),w=document.createElement('div'),h=document.createElement('h3'),m=document.createElement('p'),badge=document.createElement('span');top.className='order-top';h.className='order-title';h.textContent=r.reward_title||L('接单机会','Opportunity','ឱកាសការងារ');m.className='order-number';m.textContent=[L('积分接单','Points redemption','ការប្តូរពិន្ទុ'),`${Number(r.points_spent||2000)} ${L('积分','points','ពិន្ទុ')}`,fmt(r.created_at)].join(' · ');badge.className='status-badge delivered';badge.textContent=L('已兑换','Redeemed','បានប្តូរ');w.append(h,m);top.append(w,badge);c.append(top);const actions=document.createElement('div'),view=document.createElement('button'),down=document.createElement('button'),box=document.createElement('div'),pre=document.createElement('pre');actions.className='order-actions';view.className='btn btn-secondary';view.type='button';view.textContent=L('查看内容','View content','មើលមាតិកា');down.className='btn';down.type='button';down.textContent=L('点击下载','Download','ចុចទាញយក');box.className='purchased-answer';box.style.display='none';pre.style.whiteSpace='pre-wrap';pre.style.wordBreak='break-word';pre.textContent=r.delivery_text_snapshot||'';box.append(pre);view.onclick=()=>{const open=box.style.display!=='none';box.style.display=open?'none':'block';view.textContent=open?L('查看内容','View content','មើលមាតិកា'):L('收起内容','Hide content','លាក់មាតិកា')};down.onclick=()=>downloadOpportunity(r);actions.append(view,down);c.append(actions,box);return c}
