...
function install(){const root=$('quizOptions');if(!root)return;root.addEventListener('change',e=>{if(e.target.matches('input[type="checkbox"]'))requestAnimationFrame(syncSelected)},true);root.addEventListener('click',e=>{const label=e.target.closest('.quiz-option');if(label)setTimeout(syncSelected,0);const btn=e.target.closest('.kd-action');if(!btn)return;press(btn);if(btn.dataset.kd==='next'&&!locked){const round=window.GYX_KNOWLEDGE_DECISION?.getState?.()?.currentRound||0;locked=true;btn.classList.add('gyx-processing');btn.textContent=round===5?'已确认，正在生成最终方案…':'已确认，正在进入下一轮…';if(round===5)queueMicrotask(()=>window.dispatchEvent(new CustomEvent('gyx:final-generating')));setTimeout(()=>{locked=false;btn.classList.remove('gyx-processing')},1500)}},true);new MutationObserver(()=>requestAnimationFrame(syncSelected)).observe(root,{subtree:true,childList:true});syncSelected();
