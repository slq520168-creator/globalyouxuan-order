...
function pinIOSNav(){
 ...
 state.raf=requestAnimationFrame(()=>{
   ...
   document.querySelectorAll('.mobile-bottom-nav').forEach(n=>{
     ...
     n.style.setProperty('bottom','calc(8px + env(safe-area-inset-bottom, 0px))','important');
     n.style.setProperty('transform','translate3d(0,0,0)','important');
     n.style.setProperty('-webkit-transform','translate3d(0,0,0)','important');
...
function bindIOSViewport(){
 if(!isIOS)return;
 window.addEventListener('resize',pinIOSNav,{passive:true});
 window.addEventListener('orientationchange',pinIOSNav,{passive:true});
 window.addEventListener('pageshow',pinIOSNav,{passive:true});
 window.addEventListener('scroll',pinIOSNav,{passive:true});
 if(window.visualViewport){visualViewport.addEventListener('resize',pinIOSNav,{passive:true});visualViewport.addEventListener('scroll',pinIOSNav,{passive:true})}
}
function init(){ensureStyle();const search=document.getElementById('problemInput');if(search)search.minLength=1;keyboard();nav();bindIOSViewport();window.addEventListener('hashchange',()=>{nav();pinIOSNav()},{passive:true});if(isIOS){setTimeout(pinIOSNav,50);setTimeout(pinIOSNav,300);setTimeout(pinIOSNav,800)}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
window.GYXFoundation={nav,pinNav:pinIOSNav,refresh:()=>{ensureStyle();nav();pinIOSNav()}};
})();