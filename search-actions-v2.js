...
const SEARCH_PRODUCT_ID='knowledge-decision-standard';
...
function renderFinal(){
  ...
  if($('resultPrice'))$('resultPrice').textContent=product.product_price!=null&&product.product_price!==''?String(product.product_price):'9.90';
...
favorite?.addEventListener('click',async e=>{e.preventDefault();e.stopImmediatePropagation();clearCleanup();const m=window.GYX_CURRENT_AI_MATCH;if(!m)return toast(L('请先完成一次匹配','Complete a match first','សូមបញ្ចប់ការផ្គូផ្គងមួយសិន'));const db=window.gyxSupabase,u=await needUser();if(!u||!db){armCleanup();return}const productId=m.product?.id||SEARCH_PRODUCT_ID;favorite.disabled=true;favorite.textContent=L('收藏中…','Saving…','កំពុងរក្សាទុក…');const payload={user_id:u.id,answer_id:null,question:m.question,selections:m.selections||[],tier:m.tier||'standard',product_id:productId,quoted_price:Number(m.product?.product_price||9.9),matched_title:m.delivery_package?.title||m.answer?.title||m.question,matched_summary:m.delivery_package?.summary||m.answer?.answer_summary||m.question,updated_at:new Date().toISOString()};let timer;try{const timeout=new Promise(resolve=>{timer=setTimeout(()=>resolve({error:{message:'timeout'}}),3500)}),r=await Promise.race([db.from('answer_favorites').insert(payload),timeout]);clearTimeout(timer);if(r?.error&&!/duplicate|unique|already exists/i.test(String(r.error.message||'')))throw r.error;favorite.textContent=L('已收藏','Saved','បានរក្សាទុក');clearSearch()}catch{clearTimeout(timer);favorite.disabled=false;favorite.textContent=L('收藏','Favorite','ចំណូលចិត្ត');toast(L('收藏失败，请稍后再试','Save failed. Try again later.','រក្សាទុកបរាជ័យ សូមសាកល្បងម្តងទៀត'));armCleanup()}},true);
order?.addEventListener('click',async e=>{e.preventDefault();e.stopImmediatePropagation();clearCleanup();const m=window.GYX_CURRENT_AI_MATCH;if(!m){armCleanup();return toast(L('当前方案暂不可下单','This plan cannot be ordered yet','ផែនការនេះមិនទាន់អាចបញ្ជាទិញបាន'))}const u=await needUser();if(!u){armCleanup();return}if(!window.GYX_MEMBER_CHECKOUT?.open){armCleanup();return toast(L('下单系统正在加载，请稍后再试','Order system is loading. Try again shortly.','ប្រព័ន្ធបញ្ជាទិញកំពុងផ្ទុក សូមសាកល្បងបន្តិចទៀត'))}const productId=m.product?.id||SEARCH_PRODUCT_ID;window.GYX_MEMBER_CHECKOUT.open(productId,m);clearSearch()},true);
