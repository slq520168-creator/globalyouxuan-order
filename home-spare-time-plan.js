...
const cases=[
 {who:'每天固定时间投稿的音乐创作者',story:'他一开始和很多普通人一样，只是喜欢音乐，空闲时做一点，做完也不知道能不能变成收入。前面很长一段时间几乎没有稳定结果，但他没有因为几次没回应就停下来。后来他把每天能拿出来的一小段时间坚持留给音乐，也坚持把完成的作品送到真正有需求的人面前。慢慢地，有人回复、有人询价、有人愿意付费，原本偶尔发生的成交开始变得越来越连续。真正让人有信心的地方不是他起点有多高，而是他也是从没人找、没稳定收入、一次次尝试开始的。'},
 {who:'从零碎时间做到月收入约5万的视频创作者',story:'他最早并不是专业团队，也不是一开始就有很多客户，只是会一点剪辑，利用下班后和零碎时间做视频。最开始作品普通、客户少、收入也不稳定，但他没有一直换方向，而是把能做好的内容一点点做得更快、更稳。随着作品越来越多，客户开始愿意介绍客户，做过一次的人也会回来继续找他。后来他的收入曾做到月收入约5万元。这个结果不是每个人都会复制，但他的起点很普通：先会一点、先做一个、再做第二个，然后把一个客户慢慢变成更多客户。'},
 {who:'一个女孩用AI承接多家公司设计',story:'她开始时只是一个人，没有设计团队，也没有什么特别的资源。刚接触AI时，她也不是一上来就能做企业项目，而是先用它帮助自己完成一些原本很花时间的图片和设计工作。做得多了以后，她越来越清楚客户真正要什么，也越来越能控制最终效果。后来，一家公司变成几家公司，一个人也能同时处理不同企业的设计需求。她真正证明的不是“会AI就能赚钱”，而是普通人如果愿意把一个新工具练到能真正替别人解决问题，就有机会把原来做不到的事情慢慢做成自己的能力。'}
];
const offers=[
...
function renderOffers(){const h=host();if(!h)return;const e=buildEmotion();h.innerHTML=`<div class="gyx-profile gyx-spare-offers"><div class="gyx-profile-card"><section class="gyx-emotion"><h2>按你现在的实际情况，先选一条真正能坚持的路。</h2>${e.map(x=>`<p>${x}</p>`).join('')}</section><section class="gyx-price-area"><h2 class="gyx-profile-title">选择适合自己的节奏</h2><div class="gyx-price-grid">${offers.slice(0,5).map(o=>`<button type="button" class="gyx-price-card" data-product="${o.id}"><span>${o.name}</span><strong>${o.price}</strong><small>USDT</small><em>${o.desc}</em></button>`).join('')}</div><button type="button" class="gyx-private-card" data-product="${offers[5].id}"><span><b>${offers[5].name}</b><small>${offers[5].desc}</small></span><strong>${offers[5].price} USDT</strong></button></section><div class="gyx-profile-actions"><button type="button" class="gyx-offer-back">上一步</button><button type="button" class="gyx-gate-home">返回首页</button></div></div></div>`;$('.gyx-offer-back',h).onclick=renderCases;$('.gyx-gate-home',h).onclick=close;h.querySelectorAll('[data-product]').forEach(b=>b.onclick=async()=>{const id=b.dataset.product;if(!window.GYX_MEMBER_CHECKOUT?.open)return alert('订单功能正在加载，请稍后再点一次');await window.GYX_MEMBER_CHECKOUT.open(id,{question:'零碎时间养成计划',selections:Object.entries(profile).filter(([k])=>k!=='gate').map(([k,v])=>`${k}: ${v}`),tier:id})});screen().scrollTop=0}
