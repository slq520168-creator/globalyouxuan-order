-- Lock member-authored profile fields without blocking avatar/theme choices or
-- trusted server maintenance of membership statistics.
...
begin
  ...
  if current_user = 'authenticated' then
    ...
    -- These fields are maintained only by trusted server-side functions.
    ...
    -- Administrator or trusted server completion follows the same lock rule.
    new.profile_locked_at := clock_timestamp();
  end if;
