-- Complete profiles created by trusted registration/admin flows must start in
-- the same locked state as complete profiles updated later.
  ...
  if tg_op = 'INSERT' then
    if current_user = 'authenticated' then
      raise exception using
        errcode = '42501',
        message = 'PROFILE_INSERT_NOT_ALLOWED';
    end if;
    if profile_complete then
      new.profile_locked_at := coalesce(new.profile_locked_at, clock_timestamp());
    end if;
    return new;
  end if;
